...
public class ParabolicPointer : MonoBehaviour {
	...
	public static float SelectionMeshScale = 0.2f;
...
//        ParabolaMesh.triangles = new int[0];
		Vector3 waypointlocalscale = waypoint.transform.localScale;
		SelectionMeshScale = waypointlocalscale.x * 0.5f * 0.5f;
    ...
    void Update()
    {
		...
		/*
			Using raycast instead of parabolic pointer.
		*/
		...
		PointOnNavMesh = Physics.Raycast (transform.position, fwd, out hitinfo, 13.0f);
		SelectedPoint = hitinfo.point;

//		Debug.DrawRay (transform.position, SelectedPoint - transform.position);
        ...
        if (ShouldDrawMarker)
        {
			...
			Graphics.DrawMesh(SelectionPadMesh, Matrix4x4.TRS(SelectedPoint + Vector3.up * 0.005f, Quaternion.identity, Vector3.one * SelectionMeshScale), SelectionPadFadeMaterial, gameObject.layer, null, 3);
            // Draw Bottom of selection pad
			Graphics.DrawMesh(SelectionPadMesh, Matrix4x4.TRS(SelectedPoint + Vector3.up * 0.005f, Quaternion.identity, Vector3.one * SelectionMeshScale), SelectionPadCircleMaterial, gameObject.layer, null, 1);
            // Draw Bottom of selection pad
			Graphics.DrawMesh(SelectionPadMesh, Matrix4x4.TRS(SelectedPoint + Vector3.up * 0.005f, Quaternion.identity, Vector3.one * SelectionMeshScale), SelectionPadBottomMaterial, gameObject.layer, null, 2);
        }
...
//        Graphics.DrawMesh(ParabolaMesh, Matrix4x4.identity, GraphicMaterial, gameObject.layer);
        ...
        if (ShouldDrawMarker)
			Graphics.DrawMesh(SelectionPadMesh, Matrix4x4.TRS(SelectedPoint + Vector3.up * 0.005f, Quaternion.identity, Vector3.one * SelectionMeshScale), SelectionPadFadeMaterial, gameObject.layer, null, 0);
    }
